# Quick deployment script for Agentic SRE Copilot on EC2 (PowerShell)
...
Write-Host "Agentic SRE Copilot - EC2 Deployment" -ForegroundColor Cyan
Write-Host "==========================================" -ForegroundColor Cyan
Write-Host ""
...
if (-not $ApiKey) {
    Write-Host "❌ Error: Google API Key is required" -ForegroundColor Red
    exit 1
}
...
cat > /etc/systemd/system/sre-agent.service << 'EOFSERVICE'
[Unit]
...
# Execute deployment
try {
    ssh -i $PEM_KEY "${EC2_USER}@${EC2_HOST}" "sudo -i bash -c `"$DeployCommand`""
    
    Write-Host ""
    Write-Host "🎉 Deployment script completed!" -ForegroundColor Green
    Write-Host ""
    Write-Host "📍 To view live logs, run:" -ForegroundColor Cyan
    Write-Host "  ssh -i `"$PEM_KEY`" ${EC2_USER}@${EC2_HOST} `"sudo journalctl -u sre-agent -f`"" -ForegroundColor White
    Write-Host ""
}
catch {
    Write-Host ""
